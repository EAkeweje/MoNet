...
train_dataset = FAUST(args.data_fp,True,transform=T.Cartesian(),pre_transform=Pre_Transform())
test_dataset = FAUST(args.data_fp,False,transform=T.Cartesian(), pre_transform=Pre_Transform())
...
class MoNet(nn.Module):
    def __init__(self, in_channels, num_classes, kernel_size):
        ...
        self.conv1 = torch_geometric.nn.GMMConv(16, 32, dim=3, kernel_size=kernel_size)
        self.conv2 = torch_geometric.nn.GMMConv(32, 64, dim=3, kernel_size=kernel_size)
        self.conv3 = torch_geometric.nn.GMMConv(64, 128, dim=3, kernel_size=kernel_size)
